...
export function downLoadZip(requestUrl, filename) {
  const url = requestUrl.startsWith(baseUrl)
    ? requestUrl
    : baseUrl + requestUrl;
  axios({
    method: "get",
...
export function downloadFileByProgress(url, progressPercent, progressVisible) {
  url = url.startsWith(baseUrl) ? url : baseUrl + url;
  axios
    .get(url, {
      responseType: "blob",
      onDownloadProgress: (evt) => {
        progressPercent = Math.round((evt.loaded * 100) / evt.total);
      },
    })
    .then((res) => {
...
// 方式 B
export function downloadByServiceWorker(url, filename) {
  url = url.startsWith(baseUrl) ? url : baseUrl + url;
