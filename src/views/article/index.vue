<template>
  <!-- 文章阅读：gp 栅格 + 参考图 Hero / TOC / 作者卡 / 相关推荐 -->
  <div class="gp-page container-fluid article-page-v2">
    <div class="gp-page__inner container">
      <div class="gp-page__main">
        <!-- Hero：标题横幅 -->
        <header class="article-hero" :class="{ 'skeleton-loading': loading }">
          <div class="article-hero__bg" :style="heroBgStyle"></div>
          <div class="article-hero__content">
            <div class="article-hero__crumb" v-if="breadCrumbList && breadCrumbList.length">
              <post-breadcrumb :breadCrumbList="breadCrumbList"></post-breadcrumb>
            </div>
            <h1 class="article-hero__title">{{ articleInfo.articleTitle || '没有内容，找不到文章或主题！' }}</h1>
          </div>
        </header>

        <div class="gp-row article-body-row">
          <div class="gp-col-24 gp-col-xs-24 gp-col-sm-24 gp-col-md-17 gp-col-lg-17 gp-col-xl-17">
            <div class="main-article-content">
              ...
              <div class="article-container is-always-shadow" :class="{ 'mobile-view': isMobile }">
                ...
                <div class="article-meta-bar" :class="{ 'skeleton-loading': loading }">
                  <div class="article-meta-bar__author">
                    <img class="article-meta-bar__avatar" :src="authorAvatarSrc" alt="">
                    <div class="article-meta-bar__info">
                      <span class="article-meta-bar__name">{{ articleInfo.authorName || '佚名' }}</span>
                      <span class="article-meta-bar__time">{{ getYMDTime(articleInfo.createTime) }}</span>
                    </div>
                  </div>
                  <div class="article-meta-bar__stats">
                    <span><i class="el-icon-view"></i> {{ articleInfo.viewCount || 0 }}</span>
                    <span><svg-icon icon-class="thumb"></svg-icon> {{ articleInfo.likeCount || 0 }}</span>
                    <span><i class="el-icon-chat-round"></i> {{ commentsCount || 0 }}</span>
                  </div>
                </div>
                ...
                <div class="plus-article-toolbar">
                  ...
                  <button class="toolbar-button" type="button" @click="onLinkButtonTap">
                    ...
                    链接
                    <div id="generatelink" value="生成链接" style="display: none">生成链接</div>
                  </button>
                </div>

                <div>
                  <Adsense data-ad-client="ca-pub-7291512442295477" data-ad-slot="3776635819"></Adsense>
                </div>
                  ...
                  <div class="navigation-article-wrapper">
                    <div class="left-navigation">
                      <div v-if="prevArticle" class="navigation-info">
                        ...
                        <span>{{ prevArticle.articleTitle }}</span>
                      </div>
              ...
              <!-- 作者卡片 -->
              <div class="author-card is-always-shadow" v-if="articleInfo.authorName">
                <img class="author-card__avatar" :src="authorAvatarSrc" alt="">
                <div class="author-card__body">
                  <div class="author-card__name">{{ articleInfo.authorName }}</div>
                  <p class="author-card__bio">{{ authorBioText }}</p>
                </div>
              ...
              <div class="article-comments-container is-always-shadow" :class="{ 'mobile-view': isMobile }">
                ...
                ></comment-reply>
                <plus-pager
                  @pagination="getArticleAllUserComments"
                  :total="total"
              ...
              <!-- 底部相关推荐卡片 -->
              <div class="related-cards" v-if="!isMobile && displayRelatedCards.length">
                <h3 class="related-cards__title">相关推荐</h3>
                <div class="related-cards__grid">
                  <router-link
                    v-for="(item, idx) in displayRelatedCards"
                    :key="item.id || idx"
                    class="related-card"
                    :to="'/article/' + item.id"
                  >
                    <el-image class="related-card__cover" :src="item.indexPicture || articleCover" fit="cover"></el-image>
                    <div class="related-card__body">
                      <h4>{{ item.articleTitle }}</h4>
                      <p>{{ item.abstractText || item.articleTitle }}</p>
                    </div>
                  </router-link>
                </div>
              </div>
          ...
          <div class="gp-col-24 gp-col-xs-24 gp-col-sm-24 gp-col-md-7 gp-col-lg-7 gp-col-xl-7">
            <aside class="right-sidebar">
              ...
              <div v-if="!isMobile && tocList.length" class="gp-surface-card box-card article-toc sticky-panel">
                <div class="gp-surface-card__header">
                  <div class="category-section">
                    <span class="category-title"><i class="el-icon-notebook-2"></i>目录</span>
                  </div>
                </div>
                <div class="gp-surface-card__body">
                  <nav class="toc-nav">
                    <a
                      ...
                      href="javascript:;"
                      class="toc-nav__item"
                      :class="['is-h' + item.level, { 'is-active': activeTocId === item.id }]"
              ...
              <div v-if="!isMobile" class="gp-surface-card box-card">
                <div class="gp-surface-card__header">
                  <div class="category-section">
                    <span class="category-title"><i class="el-icon-data-board"></i>热门文章</span>
                  </div>
                </div>
                <div class="gp-surface-card__body">
                  <ol class="hot-list">
                    <li v-for="(relatedArticle, index) in relatedArticles.slice(0, 8)" :key="relatedArticle.id || index">
                      <span class="hot-list__rank" :class="'rank-' + (index + 1)">{{ index + 1 }}</span>
                      <router-link :to="'/article/' + relatedArticle.id">{{ relatedArticle.articleTitle }}</router-link>
                    </li>
                  </ol>
                </div>
              </div>
              ...
              <div v-if="!isMobile" class="gp-surface-card box-card">
                <div class="gp-surface-card__header">
                  <div class="category-section">
                    <span class="category-title"><i class="el-icon-price-tag"></i>标签云</span>
                  </div>
                </div>
                <div class="gp-surface-card__body">
                  <div class="tag-cloud">
              ...
              <div v-if="!isMobile && recommendedArticles.length" class="gp-surface-card box-card">
                <div class="gp-surface-card__header">
                  <div class="category-section">
                    <span class="category-title"><i class="el-icon-star-off"></i>推荐文章</span>
                  </div>
                </div>
                <div class="gp-surface-card__body">
...
export default {
  ...
  computed: {
    ...
    authorAvatarSrc() {
      return this.articleInfo.authorAvatar || this.articleInfo.avatar || this.userAvatar;
    },
    authorBioText() {
      return this.articleInfo.authorIntro
        || this.articleInfo.authorDesc
        || "分享技术与思考，欢迎关注交流。";
    },
    heroBgStyle() {
      const cover = this.articleInfo.indexPicture;
      if (cover) {
        return {
          backgroundImage: `linear-gradient(120deg, rgba(15, 92, 62, 0.88), rgba(26, 122, 76, 0.82)), url(${cover})`
        };
      }
      return {};
    },
    displayRelatedCards() {
      const list = (this.relatedArticles || []).filter((a) => a && a.id);
      const rec = (this.recommendedArticles || []).filter((a) => a && a.id);
      const merged = [];
      const seen = {};
      [...list, ...rec].forEach((item) => {
        if (!seen[item.id] && String(item.id) !== String(this.articleId)) {
          seen[item.id] = true;
          merged.push(item);
        }
      });
    ...
    /** 后台「打开文章」定位：/article/:id?commentId=xx */
    anchorCommentId() {
  ...
  methods: {
    onFollowAuthor() {
      this.$message({ message: "关注功能即将开放", type: "info", duration: 2000 });
    },
    buildArticleToc() {
      this.$nextTick(() => {
        ...
        headings.forEach((el, index) => {
          ...
          let id = el.id;
          if (!id) {
            id = `toc-h-${index}-${text.slice(0, 24).replace(/\s+/g, "-")}`;
            el.id = id;
          }
          const tag = (el.tagName || "H2").toUpperCase();
          list.push({
            id,
            text,
    ...
    setupTocSpy() {
      this.teardownTocSpy();
      if (!this.tocList.length || this.isMobile) return;
      this._tocScrollBound = () => {
        const offset = 120;
        let current = this.tocList[0] && this.tocList[0].id;
      ...
      window.addEventListener("scroll", this._tocScrollBound, { passive: true });
      this._tocScrollBound();
    },
    teardownTocSpy() {
      if (this._tocScrollBound) {
        window.removeEventListener("scroll", this._tocScrollBound);
        this._tocScrollBound = null;
      }
    },
    scrollToToc(id) {
    ...
    getArticleAllUserComments() {
      ...
      if (this.anchorCommentId && this.queryParams.pageSize < 50) {
...
/* —— 文章页 v2：Hero + Meta + TOC —— */
.article-page-v2 {
  --article-hero-from: #0f5c3e;
  --article-hero-to: #1a7a4c;
}
...
.article-hero {
  ...
  background: linear-gradient(125deg, var(--article-hero-from) 0%, var(--article-hero-to) 55%, #2d9b63 100%);
  box-shadow: 0 10px 28px rgba(15, 92, 62, 0.22);
}
...
.article-meta-bar__time {
  font-size: 12px;
  color: #8a919f;
}

.article-meta-bar__stats {
  display: flex;
  flex-wrap: wrap;
  gap: 12px;
  font-size: 13px;
...
.toc-nav__item {
  ...
  &.is-active {
    color: var(--article-hero-to, #1a7a4c);
    background: rgba(26, 122, 76, 0.08);
    border-left-color: var(--article-hero-to, #1a7a4c);
    font-weight: 600;
  }

  &:hover {
    color: var(--article-hero-from, #0f5c3e);
