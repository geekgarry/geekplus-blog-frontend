  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="字典编码" prop="dictCode">
        <el-input v-model="queryParams.dictCode" placeholder="请输入字典编码" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择" clearable size="small" @keyup.enter.native="handleQuery">
      ...
      <el-form-item label="删除状态" prop="delFlag">
        <el-select v-model="queryParams.delFlag" placeholder="请选择" clearable size="small" @keyup.enter.native="handleQuery">
          <el-option label="正常" :value="0"></el-option>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="字典编码" align="center" prop="dictCode">
        <template slot-scope="scope">
          <router-link :to="'/admin/system/dict-data/index/' + scope.row.id" class="link-type">{{ scope.row.dictCode
            }}</router-link>
        </template>
      </el-table-column>
      <el-table-column label="状态，0为正常，1为停用" align="center" prop="status" />
      ...
      <el-table-column label="删除状态（0正常，1已删除）" align="center" prop="delFlag" />
...
<script>
import { listSysDict, getSysDict, delSysDict, addSysDict, updateSysDict, exportSysDict, refreshCache } from "@/api/system/dict/index";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "状态，0为正常，1为停用不能为空", trigger: "blur" }
        ],
        createBy: [
        ...
        delFlag: [
          { required: true, message: "删除状态（0正常，1已删除）不能为空", trigger: "change" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getSysDict({ id: id }).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有列表数据项?', "警告", {
        ...
        return 'export' + SysDict(queryParams);
      }).then(response => {
