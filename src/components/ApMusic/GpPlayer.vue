<template>
  <div class="mainMusicPage" ref="MainMusicRef">
    <!-- <aplayer
        :autoplay="autoplay"
        :music="songInfo"
        :showLrc="showLrc"
        ...
        :preload="preload"
      ></aplayer> -->
    <div ref="APlayerRef" id="plus-a-player"></div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    * */
    ...
    /**
     *  主题色#41b883。如果当前歌曲也设置了 theme 则以歌曲的为准
     * */
    theme: {
      type: String,
      default: '#0badb6'
  ...
  data() {
    return {
      ...
      info: {
        ...
        productionTip: false,
        theme: this.theme || '#FADFA3',
      },
    }
  },
  created() {
    // keep the default list while remote music loads
  },
  async mounted() {
    await this.loadRemoteMusicAndPlayer()
  },
  methods: {
    async loadRemoteMusicAndPlayer() {
      try {
        // 引入音乐插件以及引入音乐插件的样式
        ...
        let res = null
        try {
          res = await getOnlineMusic()
        } catch (error) {
          this.$message({
            message: error.msg || '在线音乐加载失败，已使用本地备用播放列表',
            type: 'warning',
            duration: 3000
          })
        }
        ...
        if (Array.isArray(payload) && payload.length) {
          ...
          this.audio = payload.data
        } else {
          this.audio = Array.isArray(this.songList) ? this.songList : []
        }

        this.initAPlayer(APlayer)
      } catch (err) {
    ...
    initAPlayer(APlayer) {
      const ap = new APlayer({
        container: document.getElementById("plus-a-player"),
        audio: this.audio,
        ...this.info,
      })
      ap.on('play', () => {
        this.$emit('musicPlaying', true)
      })
      ap.on('playing', () => {
        this.$emit('musicPlaying', true)
      })
      ap.on('pause', () => {
        this.$emit('musicPlaying', false)
      })
    },
  },
  beforeDestroy() {
